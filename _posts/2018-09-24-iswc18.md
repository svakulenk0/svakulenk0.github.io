...
Visualisation of the embeddings layer activation demonstrates the point of view of the neural network processing the input sequences. We interpret the vertical bars, which indicate the high activation weights, stratching across the conversation as semantic coherence patterns, which help the neural network to assign a higher coherence score for positive examples, as opposed to the "white noise" pattern in the random entity sets:
...
The vertical bars continuos in the same dimension from the top to the bottom (positive example at the top) are interupted because of the topic switch (true negative example, bottom):
