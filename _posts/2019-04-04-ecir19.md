...
The final QRFA model (on the right) is produced as a sum of the individual model graphs ("as-is" model) minus the edges, which represent transitions considered as undesirable for a conversation success ("to-be" model), e.g. Q->END transition was removed. This set of edges can be identified by a human analyst from the model itself as a working hypothesis, which can be then verified by sampling from those dialogues only that contain this transition.
